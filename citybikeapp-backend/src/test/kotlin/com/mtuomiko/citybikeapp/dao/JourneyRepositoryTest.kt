...
class JourneyRepositoryTest {
    ...
    fun `Trip statistics query does not return results if no journeys for selected station`() {
        val pair = initializeStationsAndJourneysExceptForRandomTestStation(1, 3, 10)
    ...
    fun `Trip statistics query returns correct trip counts when journeys exist for selected station`() {
        val (testStationId, otherStationIds) = initializeStationsAndJourneysExceptForRandomTestStation(1, 10, 50)
    ...
    fun `Trip statistics query is limited by timestamp arguments`() {
        ...
        JourneysInitializer(journeyRepository)
            ...
            .maxDistance(42)
            .departureStationId(testStationId)
            .arrivalStationId(targetStation)
            .saveRandomJourneys(10)
            .departureStationId(targetStation)
            .arrivalStationId(testStationId)
            .saveRandomJourneys(10)
    ...
    fun `Top stations query returns empty results if no journeys for selected station`() {
        val pair = initializeStationsAndJourneysExceptForRandomTestStation(1, 10, 50)
        val testStationId = pair.first
    ...
    fun `Top stations query returns only relevant stations`() {
        ...
        journeysInitializer
            .departureStationId(testStationId)
            .arrivalStationId(targetStations[0])
            .saveRandomJourneys(5)
            .arrivalStationId(targetStations[1])
            .saveRandomJourneys(7)
            .arrivalStationId(testStationId)
            .departureStationId(targetStations[2])
            .saveRandomJourneys(9)
            .departureStationId(targetStations[3])
            .saveRandomJourneys(13)
    ...
    fun `Top stations query results are limited by timestamp arguments`() {
        ...
        JourneysInitializer(journeyRepository)
            ...
            .departureStationId(targetStation)
            .arrivalStationId(testStationId)
            .saveRandomJourneys(17)
            .earliestDeparture(earliestIncluded)
            .latestDeparture(latestIncluded)
            .departureStationId(testStationId)
            .arrivalStationId(targetStation)
            .saveRandomJourneys(19)
            .departureStationId(targetStation)
            .arrivalStationId(testStationId)
            .saveRandomJourneys(23)
            .earliestDeparture(earliestButAfterInclusion)
            .latestDeparture(latestButAfterInclusion)
            .departureStationId(testStationId)
            .arrivalStationId(targetStation)
            .saveRandomJourneys(29)
            .departureStationId(targetStation)
            .arrivalStationId(testStationId)
            .saveRandomJourneys(31)
    ...
    private fun initializeStationsAndJourneysExceptForRandomTestStation(
        firstStationId: Int,
        stationCount: Int,
        journeyCount: Int,
    ): Pair<Int, List<Int>> {
        val stationIds =
            StationsInitializer(stationRepository)
                .firstId(firstStationId)
                .count(stationCount)
                .save()
                .map { it.id }
    ...
    companion object {
        val earliestButBeforeInclusion: Instant = Instant.parse("2020-01-01T00:00:00Z")
        val latestButBeforeInclusion: Instant = Instant.parse("2020-01-05T23:59:59Z")
        val earliestIncluded: Instant = Instant.parse("2020-01-06T00:00:00Z")
        val latestIncluded: Instant = Instant.parse("2020-01-10T23:59:59Z")
        val earliestButAfterInclusion: Instant = Instant.parse("2020-01-11T00:00:00Z")
        val latestButAfterInclusion: Instant = Instant.parse("2020-01-15T23:59:59Z")
    }
}
